...
//
//             sb.AppendLine($@"
//     public static SelectQuery<{returnType}, TDialect, TypedTupleGeneratedSubqueryTable<{returnType}, TDialect>> Select<{tTypes}, TDialect>(this QueryBuilder<TDialect> builder, {colParams}) where TDialect : ISqlDialect
//         => builder.Select(new TypedTupleSelectedColumns<{tTypes}, TDialect>({colArgs}));
//
//     public static SelectQuery<{returnType}, TDialect, TypedTupleGeneratedSubqueryTable<{returnType}, TDialect>> SelectDistinct<{tTypes}, TDialect>(this QueryBuilder<TDialect> builder, {colParams}) where TDialect : ISqlDialect
...
{string.Join("\n", range.Select(n => $@"        if (columnName == _col{n}.Identifier)
    ...
    private void GenerateCode(SourceProductionContext context, ImmutableArray<ClassModel?> classes)
    {
        ...
        foreach (var model in classes.Distinct())
        {
            ...
            var sb = new StringBuilder();
            var dialect = model.Dialect ?? DialectInfo.PgSql;
...
public partial class {model.Name}: ISelection<{
    ...
    model.Name
}.SelectResult, {dialect.DialectImplType}, {model.Name}.GeneratedSubqueryTable>
{{
    public readonly record struct SelectResult({selectStructProperties});

    public static ISelectedColumns<SelectResult, {dialect.DialectImplType}, {model.Name}.GeneratedSubqueryTable> Record {{ get; }} = new GeneratedStructSelection();
    public static ISelectedColumns<{model.Name}, {dialect.DialectImplType}, {model.Name}.GeneratedSubqueryTable> Mapping {{ get; }} = new GeneratedModelSelection();
    ...
    {{
        ...
        {string.Join("\n        ", model.Properties.Select(p => $"public VirtualColumn<{p.Type}, {dialect.DialectImplType}> {p.Name} {{ get; set; }}"))}
        ...
        public GeneratedSubqueryTable(
            ...
            BaseSql = baseSql;
            AliasName = aliasName;
            {string.Join("\n            ", model.Properties.Select(p => $"this.{p.Name} = new VirtualColumn<{p.Type}, {dialect.DialectImplType}>(aliasName, {p.ColumnSql}.Identifier);"))}
        }}
    ...
    private sealed class GeneratedStructSelection : ISelectedColumns<SelectResult, {dialect.DialectImplType}, {model.Name}.GeneratedSubqueryTable>
    {{
    ...
    private sealed class GeneratedModelSelection : ISelectedColumns<{model.Name}, {dialect.DialectImplType}, {model.Name}.GeneratedSubqueryTable>
    {{
        public void BuildSql(ISqlBuilder sqlBuilder) => sqlBuilder.Append(_sql);
        ...
        {{
            ...
            var nsPrefix = string.IsNullOrEmpty(model.Namespace) ? "" : $"{model.Namespace.Replace(".", "_")}_";
            context.AddSource($"{nsPrefix}{model.Name}_generated.g.cs", SourceText.From(sb.ToString(), Encoding.UTF8));
        }
    }
    ...
    static ClassModel? GetClassModel(GeneratorSyntaxContext ctx)
    {
        ...
        // Read dialect from DbSelectAttribute
        var dbSelectAttr = symbol.GetAttributes().FirstOrDefault(a => a.AttributeClass?.Name == "DbSelectAttribute");
        var dialectTypeName = dbSelectAttr != null ? Utils.GetDialectFromAttribute(dbSelectAttr) : null;
